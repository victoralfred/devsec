...
func TestRollbackHandler_Execute_NilContext(t *testing.T) {
	...
	err := handler.Execute(nil, opts) //lint:ignore SA1012 intentionally passing nil context for testing
...
func TestRollbackHandler_GetPreviousRevision_NilContext(t *testing.T) {
	...
	handler := NewRollbackHandler(&mockHelmClient{}, nil)
	_, err := handler.GetPreviousRevision(nil, "default", "app") //lint:ignore SA1012 intentionally passing nil context for testing
	if err != ErrNilContext {
...
func TestRollbackHandler_ExecuteWithResult_NilContext(t *testing.T) {
	...
	handler := NewRollbackHandler(&mockHelmClient{}, nil)
	opts := RollbackOptions{ReleaseName: "app", Namespace: "default"}
	_, err := handler.ExecuteWithResult(nil, opts) //lint:ignore SA1012 intentionally passing nil context for testing
	if err != ErrNilContext {
